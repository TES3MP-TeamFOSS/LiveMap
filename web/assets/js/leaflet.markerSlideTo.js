...
L.Marker.include({
	...
	slideTo: function slideTo(latlng, options) {
		...
		this._startRotation = options.startRotation;
		this._endRotation = options.endRotation;
	...
	_slideTo: function _slideTo() {
		...
		if(this._endRotation!= undefined)
		{
			this.setRotationAngle(lerpRot(this._startRotation,this._endRotation,percentDone));
		}
		if (this._slideKeepAtCenter) {
...
function lerpRot(startRot, endRot, time)
{
  if((360-startRot+endRot)<(startRot-endRot))
    endRot+=360;
  else if((startRot+(360-endRot))<(endRot-startRot))
    startRot+=360;
  var lerpRot = lerp(startRot,endRot,time);
  return lerpRot;
}
